...
int main(int argc, char **argv) {
    ...
    auto nh = std::make_shared<rclcpp::Node>("plan_visualizer");
    if (argc != 2)
    {
        RCLCPP_ERROR(nh->get_logger(), "Usage: ros2 run tum_trajectory_visualizer visualize_trajectory <path_to_tum_file>");
        return -1;
    }
    std::string csv_file = argv[1]; //"/home/bigdavid/Nav2/src/odom_plot/localization_data/global_plan.csv";
    std::vector<Pose> poses = read_csv_file(csv_file);
    auto plan_pub = nh->create_publisher<visualization_msgs::msg::Marker>("/visual_plan", 10);
